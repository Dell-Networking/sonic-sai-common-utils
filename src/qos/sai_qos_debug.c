...
char *sai_qos_get_drop_type_str (sai_drop_type_t drop_type, char *str, uint_t len)
{
     STD_ASSERT(str != NULL);

     switch(drop_type) {
         case SAI_DROP_TYPE_TAIL:
             snprintf(str,len, "Tail");
             break;
         case SAI_DROP_TYPE_WRED:
             snprintf(str, len, "Wred");
             break;
         default:
             snprintf(str, len, "Unknown");
     }
...
sai_status_t sai_dump_queue_node(dn_sai_qos_queue_t *queue_node)
{
    ...
    char drop_type_str[SAI_MAX_DROP_TYPE_STRLEN];
    ...
    SAI_DEBUG ("Drop type         : %s", sai_qos_get_drop_type_str (queue_node->drop_type,
                                       drop_type_str, SAI_MAX_DROP_TYPE_STRLEN));
...
{
    char drop_type_str[SAI_MAX_DROP_TYPE_STRLEN] = {0};
    ...
    SAI_DEBUG ("---------------------------------------------");
    SAI_DEBUG ("Drop type        : %s", sai_qos_get_drop_type_str (port_node->drop_type,
                                       drop_type_str, SAI_MAX_DROP_TYPE_STRLEN));
