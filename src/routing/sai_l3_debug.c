...
void sai_fib_dump_route_node (sai_fib_route_t *p_route)
{
    ...
    SAI_DEBUG ("IP Prefix: %s/%d, VRF: 0x%"PRIx64", NH obj Type: %s, "
               "NH Obj Id: 0x%"PRIx64", Packet-action: %s, Trap Prio: %d.",
               sai_ip_addr_to_str (&p_route->key.prefix, addr_str,
                                  ...
                                  sai_object_id_t rif_id)
{
    ...
    p_nh_node = sai_fib_ip_next_hop_node_get (rif_id, &ip_addr);
